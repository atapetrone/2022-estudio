...
function avril_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'avril-pro' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'avril-pro' ),
...
function avril_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		$categories_list = get_the_category_list( esc_html__( ', ', 'avril-pro' ) );
		if ( $categories_list && avril_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'avril-pro' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'avril-pro' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'avril-pro' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'avril-pro' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
...
function avril_custom_excerpt_length( $length ) {
	 $avril_post_excerpt = get_theme_mod('avril_post_excerpt','55');
    if( $avril_post_excerpt == 1000 ) {
        return 9999;
    }
    return esc_html( $avril_post_excerpt );
}


/**
 * Avril Header Widget Area First
 */
function avril_header_widget_area_first() {
	$avril_header_widget_first = 'avril-header-widget-left';
	if ( is_active_sidebar( $avril_header_widget_first ) ){ 
		dynamic_sidebar( 'avril-header-widget-left' );
	} elseif ( current_user_can( 'edit_theme_options' ) ) {

			$avril_sidebar_name = avril_get_sidebar_name_by_id( $avril_header_widget_first );
			?>
			<div class='widget widget_none'>
				<h4 class='widget-title'><?php echo esc_html( $avril_sidebar_name ); ?></h4>
				<p>
					<?php if ( is_customize_preview() ) { ?>
						<a href="#" class="" data-sidebar-id="<?php echo esc_attr( $avril_header_widget_first ); ?>">
					<?php } else { ?>
...
/**
 * Avril Header Widget Area Second
 */
function avril_header_widget_area_second() {
	$avril_header_widget_first = 'avril-header-widget-right';
	if ( is_active_sidebar( $avril_header_widget_first ) ){ 
		dynamic_sidebar( 'avril-header-widget-right' );
} elseif ( current_user_can( 'edit_theme_options' ) ) {
