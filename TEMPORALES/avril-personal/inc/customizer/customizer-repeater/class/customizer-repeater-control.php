...
class AVRIL_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'avril-pro' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'avril-pro' );
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'avril_customizer-repeater-admin-stylesheet', get_template_directory_uri() . '/inc/customizer/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'avril_customizer-repeater-script', get_template_directory_uri() . '/inc/customizer/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true  );

		wp_enqueue_script( 'avril_customizer-repeater-fontawesome-iconpicker', get_template_directory_uri() . '/inc/customizer/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'avril_customizer-repeater-fontawesome-iconpicker-script', get_template_directory_uri() . '/inc/customizer/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );
	}
		...
		?>
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						<?php
						$choice = $image_url = $icon_value = $title = $subtitle = $text = $text2 =  $link2 = $button_second = $link = $designation = $slide_align = $button = $open_new_tab = $shortcode = $repeater = $color = $video_url = '';
						if(!empty($icon->id)){
							$id = $icon->id;
						}
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Title','avril-pro' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Subtitle','avril-pro' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Description','avril-pro' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if($this->customizer_repeater_text2_control==true){
                            $this->input_control(array(
                                'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Button Label','avril-pro' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if($this->customizer_repeater_button2_control==true){
                            $this->input_control(array(
                                'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Button Label second','avril-pro' ), $this->id, 'customizer_repeater_button2_control' ),
                                'class' => 'customizer-repeater-button2-control',
                                'type'  => apply_filters('avril_repeater_input_types_filter', '', $this->id, 'customizer_repeater_button2_control' ),
                            ), $button_second);
                        }
						
						if($this->customizer_repeater_link2_control){
                            $this->input_control(array(
                                'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'link','avril-pro' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if($this->customizer_repeater_button_text_control){
							...
							'avril-pro'), $this->id, 'customizer_repeater_button_text_control'),
						...
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Link','avril-pro' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if($this->customizer_repeater_video_url_control){
							...
							'avril-pro'), $this->id, 'customizer_repeater_video_url_control'),
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Color','avril-pro' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Shortcode','avril-pro' ), $this->id, 'customizer_repeater_shortcode_control' ),
						...
						if($this->customizer_repeater_designation_control==true){
							$this->input_control(array(
								'label' => apply_filters('avril_repeater_input_labels_filter', esc_html__( 'Designation','avril-pro' ), $this->id, 'customizer_repeater_designation_control' ),
						...
						<button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php printf( __( 'Delete %s', 'avril-pro' ), $this->boxtitle );	?>
						</button>
			...
			<div class="customizer-repeater-general-control-repeater-container">
				...
				<div class="customizer-repeater-box-content-hidden">
					...
					<button type="button" class="social-repeater-general-control-remove-field button" style="display:none;">
						<?php esc_html_e( 'Delete field', 'avril-pro' ); ?>
					</button>
				</div>
			</div>
			<?php
...
//print_r($options);
	...
	<?php esc_html_e('Open link in new tab:','avril-pro'); ?>
	<span class="switch">
	...
	private function icon_picker_control($value = '', $show = '', $class=''){ ?>
		<div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo esc_attr('style="display:none;"'); } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','avril-pro'); ?>
            </span>
			<span class="description customize-control-description">
                <?php
                echo sprintf(
	                esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'avril-pro' ),
	                sprintf( '<a href="http://fontawesome.io/icons/" rel="nofollow">%s</a>', esc_html__( 'http://fontawesome.io/icons/', 'avril-pro' ) )
                ); ?>
            </span>
		...
		<div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' ) { echo esc_attr('style="display:none;"'); } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Image','avril-pro')?>
            </span>
			...
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button <?php if($class="avril_overlimit") { echo esc_attr('avril-uploading-img-btn');}?> <?php echo esc_attr($auto);?>" value="<?php esc_attr_e( 'Upload Image','avril-pro' ); ?>" />
		</div>
		<?php
	}
	...
	private function icon_type_choice($value='customizer_repeater_icon', $avril_limit=''){ ?>
		<span class="customize-control-title">
            <?php esc_html_e('Image type','avril-pro');?>
        </span>
		<select class="customizer-repeater-image-choice  <?php echo esc_attr($avril_limit);?>">
			<option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','avril-pro'); ?></option>
			<option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_image');?>><?php esc_html_e('Image','avril-pro'); ?></option>
			<option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','avril-pro'); ?></option>
		</select>
		<?php
	}
	...
	private function repeater_control($value = '', $avril_limit='', $avril_type_with_id=''){
		...
		$show_del        = 0; ?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'avril-pro' ); ?></span>
		<?php
		if(!empty($value)) {
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) { ?>
			<div class="customizer-repeater-social-repeater">
				<div class="customizer-repeater-social-repeater-container">
					...
					<input type="text" class="customizer-repeater-social-repeater-link team_linkdata_<?php echo $avril_limit;?> <?php echo esc_attr($avril_type_with_id);?>"
					       placeholder="<?php esc_attr_e( 'Link', 'avril-pro' ); ?>">
					<input type="hidden" class="customizer-repeater-social-repeater-id" value="">
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'avril-pro' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary "><?php esc_html_e( 'Add Icon', 'avril-pro' ); ?></button>
			<?php
		} else { ?>
			<div class="customizer-repeater-social-repeater">
				<?php
				foreach ( $social_repeater as $social_icon ) {
					...
					<div class="customizer-repeater-social-repeater-container">
						...
						<button class="social-repeater-remove-social-item"
						        ...
						        } ?>"><?php esc_html_e( 'Remove Icon', 'avril-pro' ); ?></button>
					</div>
					<?php
				} ?>
			...
			<button class="social-repeater-add-social-item button-secondary <?php echo esc_attr($avril_limit);?> <?php echo esc_attr($avril_type_with_id);?>"><?php esc_html_e( 'Add Icon', 'avril-pro' ); ?></button>
			<?php
		}
	}
}